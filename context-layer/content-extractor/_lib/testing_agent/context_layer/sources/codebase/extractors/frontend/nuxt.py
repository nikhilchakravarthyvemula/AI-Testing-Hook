...
_TEMPLATE_RE = re.compile(r"<template\b[^>]*>(.*?)</template>", re.DOTALL)


class NuxtExtractor:
    """Nuxt 3 routes + interactions + form fields."""

    name: str = "nuxt"
    discovery_tier: DiscoveryTier = DiscoveryTier.CODE_REGEX
    file_globs: tuple[str, ...] = ("*.vue",)

    def supports(self, file: Path, content: str) -> bool:
        return file.suffix == ".vue" and "pages" in file.parts
    ...
    def extract(self, file: Path, content: str) -> ExtractionOutput:
        ...
        markup = template_match.group(1) if template_match else content
