...
Catches what the deterministic SQLAlchemy parser (../db-schema/) can't:
  * Django ORM models
...
our standard `db-schema-llm.json` bundle.

Pairs with `../db-schema/extract.py` (the deterministic SQLAlchemy
extractor). Both write to `output/code-extractors/<id>.json`; the
indexer's `db-schema` topic reads both and merges by table name.
...
# parents: [0]=db-schema-llm/ [1]=code-extractors/ [2]=content-extractor/
#          [3]=context-layer/ [4]=<repo>
REPO_ROOT = Path(__file__).resolve().parents[4]
OUT_FILE  = REPO_ROOT / "output" / "db-schema" / "llm.json"
ASK_PY    = REPO_ROOT / "infrastructure" / "agentic-harness" / "bin" / "ask.py"
AGENTIC_PY = REPO_ROOT / "scripts" / "graphify" / ".venv" / "bin" / "python"
...
def _assert_subprocess_available() -> None:
    ...
    if not AGENTIC_PY.is_file():
        print(
            ...
            f"  (expected scripts/graphify/.venv/ with openharness installed)",
            file=sys.stderr,
        )
        sys.exit(2)
