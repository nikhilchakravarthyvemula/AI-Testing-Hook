"""framework-extractor entrypoint.
...
detected frameworks with `EXTRACTOR_CATALOG`, and writes the result to:
  ...
  output/code-extractors/framework-detection.json
...
def _parse_cli() -> tuple[Path | None, LLMTrigger]:
    ...
    """
    parser = argparse.ArgumentParser(prog="framework-extractor", add_help=True)
...
def main() -> int:
    ...
    out_path = _repo_root() / "output" / "code-extractors" / "framework-detection.json"
    ...
    if target is None:
        ...
        print(f"[framework-extractor] {skip_reason}", file=sys.stderr)
    ...
    print(
        f"[framework-extractor] scanning {target} with detector={detector.name} "
    ...
    print(
        f"[framework-extractor] detected: "
        f"languages={[L['name'] for L in bundle['languages']]} "
        f"frameworks={[F['name'] for F in bundle['frameworks']]}",
        flush=True,
    )
    print(
        f"[framework-extractor] recommended extractors ({len(bundle['recommendedExtractors'])}): "
        f"{bundle['recommendedExtractors']}",
        flush=True,
    )
    print(
        f"[framework-extractor] wrote {out_path.relative_to(_repo_root())} "
