...
How it works:
  1. Spawn `infrastructure/agentic-harness/bin/ask.py` as a subprocess.
...
# This file: <repo>/context-layer/content-extractor/framework-extractor/framework_extractor/llm_detector.py
#   parents[0]=framework_extractor/  parents[1]=framework-extractor/
#   parents[2]=content-extractor/    parents[3]=context-layer/
#   parents[4]=<repo>
_REPO_ROOT = Path(__file__).resolve().parents[4]
_ASK_PY = _REPO_ROOT / "infrastructure" / "agentic-harness" / "bin" / "ask.py"
_AGENTIC_PYTHON = _REPO_ROOT / "scripts" / "graphify" / ".venv" / "bin" / "python"
    ...
    def _assert_subprocess_available(self) -> None:
        ...
        if not self._python_bin.is_file():
            raise RuntimeError(
                ...
                f"{self._python_bin} (expected scripts/graphify/.venv with openharness installed)"
            )
