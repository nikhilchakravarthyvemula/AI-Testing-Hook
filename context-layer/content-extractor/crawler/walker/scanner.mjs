...
export async function scanInteractables(page, opts) {
  const { safeRe, destrRe, sameOrigin = null, cursorWalkMax = 5000, username = '' } = opts || {};

  return page.evaluate(({ safeRe, destrRe, sameOrigin, cursorWalkMax, username }) => {
    ...
    // Username tokens — derived from the logged-in account (LOGIN_EMAIL).
    // An account / avatar / user-menu control almost always shows the
    // username or email, and expanding it exposes the Sign-out action. We
    // treat such controls as destructive (don't click) to avoid logging the
    // walker out mid-crawl — complementing the logout/signout regex. Tokens:
    // the full email, its local part, and the local part split on . _ - .
    // Only tokens ≥ 3 chars are used (avoid matching short noise).
    const userTokens = (() => {
      const u = (username || '').trim().toLowerCase();
      if (!u) return [];
      const local = u.split('@')[0] || '';
      const parts = [u, local, ...local.split(/[._-]+/)];
      return [...new Set(parts.filter(t => t && t.length >= 3))];
    })();
    ...
    const items = [];
    const rejected = { destructive: 0, account: 0, notSafe: 0, inForm: 0, disabled: 0, invisible: 0 };
    let domIndex = 0;
    ...
    for (const el of semanticSet) {
      ...
      if (kind === 'nav' || kind === 'click') {
        ...
        // Username/account filter: a control whose label shows the logged-in
        // user (avatar / account menu) is the gateway to Sign-out — skip it.
        // Exempt row-like elements so data rows that merely *contain* an email
        // (e.g. a members table) stay clickable for drill-down.
        if (userTokens.length && lab.userSignal) {
          const sig = lab.userSignal.toLowerCase();
          const isRow = tag === 'tr' || role === 'row' || role === 'gridcell' || role === 'listitem';
          if (!isRow && userTokens.some(t => sig.includes(t))) {
            rejected.account++; base.rejected = 'account'; items.push(base); continue;
          }
        }
  ...
  }, { safeRe, destrRe, sameOrigin, cursorWalkMax, username }).catch(() => ({
