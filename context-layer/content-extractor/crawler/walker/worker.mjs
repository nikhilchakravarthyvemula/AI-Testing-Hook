  ...
  while (!state.isBudgetExhausted()) {
    ...
    try {
      ...
      if ((process.env.CRAWL_DIAG_SCREENSHOTS ?? '0') === '1') {
        try {
          // Resolve against REPO_ROOT (derived from import.meta.url), not cwd.
      ...
      const scan = await scannerFn(page, { safeRe, destrRe, sameOrigin, username, log });
      ...
      // groups have size 1.
      // The structural cap applies to TABLE/LIST/GRID ROWS ONLY. A non-row
      // repeated control (toolbar buttons, filter chips, tabs) is left
      // uncapped — page-dedup + global-claim already collapse exact repeats,
      // and we don't want to silently drop distinct one-off actions. Only the
      // big-N row structures (tables, virtual lists, card grids) get sampled.
      const isRowLike = (it) =>
        it.tag === 'tr'
        || ['row', 'gridcell', 'listitem'].includes(it.role)
        || /(?:>\s*tbody[^>]*>\s*tr\b|\[role=["']?(?:row|gridcell|listitem)["']?\]|>\s*li:nth-of-type|>\s*li\b)/i.test(it.selector || '');
