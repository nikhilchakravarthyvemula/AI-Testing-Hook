...
// MiniMax-M2.7 is a REASONING model — it emits a long <think> block before the
// JSON, so a 25s budget routinely timed out and silently fell back to
// heuristics (the root cause of "the LLM isn't really being used"). The prompt
// is tiny (≤4KB DOM excerpt); 90s comfortably covers the reasoning latency.
...
const SYSTEM_PROMPT = (
  ...
  '  3. (normal_login) the EXACT CSS selectors for the email, password and ' +
  'submit elements ON THIS PAGE (read them from the DOM excerpt — prefer ' +
  'stable name/id/type attributes).\n' +
  '  4. (sso_redirect) the STEPS the crawler executes.\n\n' +
...
const SCHEMA_HINT = `{
  ...
  "fields": {
    // for normal_login: the actual selectors on THIS page (omit/null otherwise)
    "emailSelector": "<CSS, e.g. input[name='username']>",
    "passwordSelector": "<CSS, e.g. input[type='password']>",
    "submitSelector": "<CSS or text=… for the submit button>"
  },
  "steps": [
...
export function validate(raw) {
  ...
  // Concrete per-page selectors for normal_login (string-typed, optional).
  const rf = (raw.fields && typeof raw.fields === 'object') ? raw.fields : {};
  const asSel = (v) => (typeof v === 'string' && v.trim() ? v.trim() : null);
  const fields = {
    emailSelector:    asSel(rf.emailSelector),
    passwordSelector: asSel(rf.passwordSelector),
    submitSelector:   asSel(rf.submitSelector),
  };
