...
- `scripts/graphify/` — deterministic AST nodes + edges + semantic enrichment for code.
...
A pragmatic split: **use NetworkX (pure, deterministic, free) for analytics over all existing graphs immediately**; introduce **kg-gen (LLM, costs tokens, nondeterministic) only for text sources**. Node.js note: you can run NetworkX as a small Python sidecar reading `click-graph.json` (you already ship a `.venv` under `scripts/graphify/` and a Python AST extractor — there's precedent for Python in this repo), or use `graphology` + `graphology-metrics` to stay in-process in Node.

### Watch-outs

- kg-gen output is nondeterministic and costs tokens — keep it out of the hot path and cache by source hash.
- Don't let an LLM-extracted graph overwrite your deterministic AST/crawl facts; merge with your existing `DiscoveryTier`/confidence model (`knowledge-base/schema.mjs`), keeping LLM triples at a lower tier.
...
3. **Store as files, git-tracked.** Write under `knowledge-sources/tribal-knowledge/` (or a new `skills/learned/`). Markdown = diffable, reviewable, portable — matches agent-qa's "review QA like code" philosophy too.
...
- Self-healing, execution memory, action cache, sandboxed hooks: **missing.** Your `generator/` emits static `@playwright/test` specs; `infrastructure/container-deploy/` is deferred.
...
| Action/plan cache | `infrastructure/` (next to model-connector cache) | Key validated step plans by test signature; your diagram already lists a `cache` on `ModelAPIConnector` but it isn't a plan cache yet. |
...
| Sandboxed hooks | finally implement `infrastructure/container-deploy/` | Docker hooks for fixture seeding / teardown. |
