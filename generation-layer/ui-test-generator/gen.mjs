...
// `npm run login` (testo/src/crawler/login-once.mjs → output/crawler/auth-state.json).
...
if (authType === 'storageState' && !hasState) {
  ...
  console.warn(`    One-time interactive login required for SSO targets:  (cd testo/src/crawler && npm run login)`);
