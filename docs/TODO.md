...
2. **New script: `context-layer/content-extractor/crawler/classify.mjs`.** Reads `pages.json`, sends each button (page URL + text + aria-label + any nearby heading) to a local model with a prompt like:
...
3. **New script: `context-layer/content-extractor/crawler/safe-click.mjs`.** Reads `button-classification.json`, for each button labeled `safe-read` or `safe-nav` (above confidence threshold): navigate to the page, click the button, capture the network traffic into the same raw streams (appending to existing run).
...
- ✅ SSO / OAuth support via `login-once.mjs` — interactive login, saves `storageState`, `crawl.mjs` auto-injects on subsequent runs. Handles email-only first / multi-step IDP flows. `npm run login`.
