...
Previously, `testo/harness/agentic_harness/tools/graphify/tool.py` constructed its own `QueryEngine`
...
```bash
python testo/harness/bin/call_tool.py graphify \
       --mode direct \
...
Same priority chain as `testo/harness/agentic_harness/tools/graphify/tool.py:_auto_backend`:
...
2. Add a row to `registry.py:_TOOLS`:
   ...
   ```python
   _TOOLS = {
       "graphify":   ("testo/harness/agentic_harness/tools/graphify/tool.py", "GraphifyTool", "GraphifyArgs"),
...
Reuses the venv at `context-layer/content-extractor/graphify/.venv/` (which has OpenHarness
installed). The graphify wrapper invokes `call_tool.py` with that
interpreter:

```bash
context-layer/content-extractor/graphify/.venv/bin/python testo/harness/bin/call_tool.py …
```
