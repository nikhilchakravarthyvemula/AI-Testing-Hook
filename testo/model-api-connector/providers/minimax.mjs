...
// ── token-usage sink (temporary, for a token-utilization measurement run) ────
// Appends one JSON line per MiniMax call. Never throws (a logging failure must
// not break an LLM call). Repo root is three levels up from this provider.
const _USAGE_LOG = path.resolve(
  path.dirname(fileURLToPath(import.meta.url)), '..', '..', '..',
  'output', 'minimax-usage.jsonl',
);
function _logUsage(model, u) {
  try {
    fs.mkdirSync(path.dirname(_USAGE_LOG), { recursive: true });
    fs.appendFileSync(_USAGE_LOG, JSON.stringify({
  ...
  } catch { /* never break a call over telemetry */ }
}
...
export function createMiniMaxClient(opts = {}) {
  ...
  return {
    ...
    async chat(req) {
      ...
      _logUsage(raw?.model ?? body.model, _u);
